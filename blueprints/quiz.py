...
from translations.translations_quiz import QUIZ_TRANSLATIONS, trans, get_translations
...
# Hardcoded badges with score ranges
BADGES = [
    {
        'key': 'starter',
        'min_score': 0,
...
def assign_badges_quiz(user_df, all_users_df, language='en'):
    ...
    if user_df.empty:
        ...
        return [trans('badge_unranked', lang=language)]
    ...
    if not badges:
        ...
        badges.append(trans('badge_unranked', lang=language))
    ...
    try:
        msg = Message(
            ...
            html=render_template(
                'quiz_email.html',
                trans=trans,
                user_name=user_name or trans('core_user', lang='en'),
...
def quiz_step(step_num):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if session['quiz_data'].get('send_email') and session['quiz_data'].get('email'):
            ...
            flash(trans('quiz_check_inbox_success', lang=language), 'success')
