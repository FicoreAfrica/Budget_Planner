...
def dashboard():
    ...
    is_overview = not any([request.args.get('tool_name'), request.args.get('start_date'), request.args.get('end_date'), request.args.get('action')])
    ...
    try:
                ...
                for date, action, count in daily_usage:
                    while current_date < datetime.strptime(date, '%Y-%m-%d'):
                        ...
                        for act in chart_data['usage_counts']:
                            chart_data['usage_counts'].setdefault(act, []).append(0)
                        current_date += timedelta(days=1)
                        idx += 1
        ...
        return render_template(
            'admin_dashboard.html',
            lang=lang,
            metrics=[] if not is_overview else {},
            chart_data={},
            valid_tools=VALID_TOOLS[3:],
            tool_name=None,
            start_date=None,
