...
def overview():
    ...
    try:
        ...
        multi_tool_users = db.session.query(ToolUsage.user_id, func.count(func.distinct(ToolUsage.tool_name)))\
            ...
            .having_count(func.count(func.distinct(ToolUsage.tool_name)) > 1)\
            .count()
        ...
        logger.info(f"Admin dashboard overview accessed by {current_user.username}")
        ...
        flash(trans('admin_error', default='An error occurred while loading the dashboard'), 'danger')
        return render_template('admin_dashboard.html', lang=lang, metrics={}, [], valid_tools=VALID_TOOLS), 500
...
def tool_usage():
    ...
    try:
        ...
        logger.info(f"Tool usage analytics accessed by {current_user.username}, tool={tool_name}, start={start_date_str}, end={end_date_str}")
        return render_template(
            'admin_dashboard.html',
            lang=lang,
            metrics=usage_logs,
        ...
        logger.error(f"Error in tool usage analytics: {str(e)}")
        flash(trans('admin_error', default='An error occurred while loading analytics'), 'danger')
        return render_template('admin_dashboard.html', lang=lang, metrics= [], valid_tools=VALID_TOOLS), 500
...
def export_csv():
    ...
    try:
        ...
        logger.info(f"CSV export generated by {current_user.username}, tool={tool_name}, start={start_date_str}, end={end_date_str}")
        return Response(
            output,
            mimetype='text/csv',
        ...
        flash(trans('admin_export_error', default='An error occurred while exporting CSV'), 'danger')
