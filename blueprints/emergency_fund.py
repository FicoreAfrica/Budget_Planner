    ...
    try:
        if request.method == 'POST' and form.validate_on_submit():
            ...
            if step3_data['risk_tolerance_level'] == 'high':
                recommended_months = max(0, months, 12)
            elif step3_data['risk_tolerance_level'] == 'low':
                recommended_months = min(6, months, 6)
            ...
            record = {
                ...
                'data': {
                    ...
                    'percent_of_income': percentage_of_income_needed,
                    'badges': badges,
                    'created_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                }
            }
            emergency_fund_storage = current_app.config['STORAGE_MANAGERS']['emergency_fund']
            emergency_fund_storage.append(record, user_email=step1_data.get('email') if step1_data['email'] else None, session_id=session['sid'])
            if step1_data['email_opt_in'] and step1_data['email']:
                send_email(
                    ...
                    data={
                        ...
                        'dependents': step3_data.get('dependents', 0),
                        'months': months,
                        'recommended_months': recommended_month,
                        'target_amount': target,
                        'savings_gap': gap,
                        'monthly_savings_amount': monthly_savings,
                        'percentage_of_income_needed': percentage_of_income,
                        'badges': badges,
            ...
            flash(trans('emergency_fund_completed_successfully'), 'success')
        ...
        current_app.logger.exception(f"Error in step4: {e}")
        flash(trans('an_unexpected_error_occurred'), 'danger')
        return render_template('emergency_fund_step4', form=form, step=4, trans=trans, lang=lang)
...
def dashboard():
    ...
    try:
        ...
        if not user_data and 'emergency_fund_step1' in session and session['emergency_fund_step1'].get('email'):
            email = user_data['emergency_fund_step1']['email']
                        ...
                        if latest_budget.get('monthly_income') and latest_budget.get('monthly_expenses'):
                            ...
                            return render_template(
                                ...
                                tips=[
                                    ...
                                    trans('budget_tip_monthly_savings'),
                                ],
                                trans=trans,
                                lang=lang
                            )
                            ...
                            flash(trans('emergency_fund_load_dashboard_error'), 'danger')
                            return render_template(
                                'emergency_fund_dashboard.html',
                                records=[],
                                latest_record={'},
                                ...
                                cross_tool_insights=[],
                                tips=[
                                    trans('emergency_fund_tip_automated_savings'),
                                    trans('budget_tip_ajo_savings'),
