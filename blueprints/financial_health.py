        ...
        if not health_record:
            ...
            if not stored_records:
                ...
                if final_record.get('data') and 'step' in final_record['data']:
                    ...
                    latest_record = final_record.get('data', {}) # Assume data is directly under 'data' key if no 'step'
            ...
            for field in required_fields:
                if field not in latest_record or latest_record[field] is None:
                    ...
                    try:
                        ...
                        latest_record[field] = 0
            records = [(str(uuid.uuid4()), latest_record)] # Assign a dummy ID for session-only record
        ...
        if latest_record:
            try:
                ...
                if total_users >= 5:
                    ...
                    insights.append(t("Not enough users for detailed comparison yet. Invite others to join!"))
