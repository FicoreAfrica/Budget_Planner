    ...
    try:
        if request.method == 'POST':
            if form.validate_on_submit():
                is_admin = form.email.data == os.environ.get('ADMIN_EMAIL', 'abumeemah@gmail.com')
                ...
                db.session.add(user)
                db.session.commit()
                logger.info(f"User signed up: {user.username} with referral code: {referral_code or 'none'}, role: {role}, is_admin: {is_admin}", extra={'session_id': session_id})
